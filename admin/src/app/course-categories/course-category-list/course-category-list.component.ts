...
@Component({
  selector: "admin-course-category-list",
...
export class CourseCategoryListComponent implements OnInit {
  ...
  refresh(state: ClrDatagridStateInterface) {
    ...
    this.repository.fetch("course_category", state).subscribe((result: any) => {
  ...
  onDelete() {
    this.loading = true;
    this.repository
      .delete("course_category", this.getSelectedIds())
  ...
  onRestore() {
    this.loading = true;
    this.repository
      .restore("course_category", this.getSelectedIds())
      .subscribe((result: any) => {
        this.refresh(this.lastState);
        this.selected = [];
      });
