...
create table course_instructor (
  ...
  customer_id uuid references customer(id) on update cascade,
  constraint course_has_instructor_id primary key (course_id, customer_id)
);
...
create table course_reservation (
  ...
  created_at      timestamptz not null default now(),
  updated_at      timestamptz not null default now(),
  constraint customer_has_course_reservation_id primary key (course_id, customer_id)
);
...
create table course_reservation_history (
  reservation     text references course_reservation(id),
  status          text not null,
  created_at      timestamptz not null default now()
);
